...
export default function SelectionInput(props) {
    ...
    const handleSelectItem = (item) => {
        setIsSelected(true)
        let detail = item?.details_ratio.map((detail, index) => {
            return " " + detail.size.name + ":" + detail.qty
        })

        onItemSelected(item)
        setSelected(item.name + " (" + detail + ")")
        setIsOpen(false)
    }
    ...
    const fetch = (q = '') => {
        setLoading(true)
        axios.get(route('api.ratios.index', { 'q': q, 'all': all }), {
            headers: {
                                                        ...
                                                        <div className="w-full items-center flex">
                                                            <div className="mx-2">
                                                                <span>
                                                                    ...
                                                                    {item.name} {"(" + item?.details_ratio.map((detail, index) => {
                                                                        return " " + detail.size.name + ":" + detail.qty
                                                                    }) + ")"}
                                                                </span>
                                                            </div>
